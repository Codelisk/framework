...
namespace Shiny.Extensions.Dialogs.XfMaterial
{
    public class XfMaterialDialogs : IDialogs
    {
        ...
        private readonly ILocalizationSource? localizeSource;
        ...
        {
            var task = await platform.InvokeTaskOnMainThread(() => MaterialDialog.Instance.SelectChoiceAsync(
                title,
                actions.ToList(),
                acceptText,
                dismissText,
        ...
        public virtual Task Alert(string message, string? title = null, string? dismissText = null)
            => platform.InvokeTaskOnMainThread(() => MaterialDialog.Instance.AlertAsync(
                message,
                title,
        ...
        {
            var result = await platform.InvokeTaskOnMainThread(async () => await MaterialDialog.Instance.ConfirmAsync(
                message,
                title,
        ...
        public Task<string?> Input(string question, string? title = null, string? acceptText = null, string? dismissText = null, string? placeholder = null, int? maxLength = null)
            => platform.InvokeTaskOnMainThread(() => MaterialDialog.Instance.InputAsync(
                title,
                question,
                null,
                placeholder,
        ...
        {
            var dialog = await platform.InvokeTaskOnMainThread(() => MaterialDialog.Instance.LoadingSnackbarAsync(message));
            return AsyncDisposable.Create(async () => await platform.InvokeTaskOnMainThread(() => dialog.DismissAsync()));
        }


        public async Task<IAsyncDisposable> LoadingDialog(string message)
        {
            var dialog = await platform.InvokeTaskOnMainThread(() => MaterialDialog.Instance.LoadingDialogAsync(message));
            return AsyncDisposable.Create(async () => await platform.InvokeTaskOnMainThread(() => dialog.DismissAsync()));
        }
        ...
        {
            var localizedText = localizeSource?[text];
